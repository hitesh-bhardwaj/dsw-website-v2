...
const UserIcon = () => (
  ...
  >
    <path
      d="M40.7695 21C40.7695 31.9184 31.9184 40.7695 21 40.7695C10.0814 40.7695 1.23047 31.9184 1.23047 21C1.23047 10.0814 10.0814 1.23047 21 1.23047C31.9184 1.23047 40.7695 10.0814 40.7695 21Z"
      stroke="#1727FF"
      strokeWidth="2.46094"
      strokeMiterlimit="10"
      strokeLinecap="round"
    ...
    />
    <path
      d="M28.9075 18.3637C28.9075 22.731 25.3672 26.2715 20.9997 26.2715C16.632 26.2715 13.0918 22.731 13.0918 18.3637C13.0918 13.9964 16.632 10.4559 20.9997 10.4559C25.3672 10.4559 28.9075 13.9964 28.9075 18.3637Z"
      stroke="#1727FF"
      strokeWidth="2.46094"
      strokeMiterlimit="10"
      strokeLinecap="round"
    ...
    />
    <path
      d="M9.20508 36.866C9.83754 30.9104 14.8769 26.2711 21.0001 26.2711C27.1235 26.2711 32.1628 30.9107 32.795 36.8662"
      stroke="#1727FF"
      strokeWidth="2.46094"
      strokeMiterlimit="10"
      strokeLinecap="round"
...
const ShieldIcon = () => (
  ...
  >
    <path
      d="M35.2461 10.8142C35.2245 9.68027 35.2039 8.60905 35.2039 7.57272C35.2039 6.76083 34.5459 6.10262 33.7338 6.10262C27.4467 6.10262 22.6598 4.29576 18.6698 0.41626C18.099 -0.138851 17.1906 -0.138655 16.62 0.41626C12.6304 4.29576 7.84431 6.10262 1.55756 6.10262C0.745671 6.10262 0.0874565 6.76083 0.0874565 7.57272C0.0874565 8.60925 0.0670711 9.68085 0.0453135 10.815C-0.156189 21.3676 -0.432175 35.8199 17.1634 41.9188C17.3194 41.9729 17.4821 42 17.6448 42C17.8075 42 17.9704 41.9729 18.1262 41.9188C35.7232 35.8197 35.4476 21.367 35.2461 10.8142ZM17.645 38.9698C2.55449 33.4879 2.78265 21.4791 2.98513 10.871C2.99728 10.2344 3.00904 9.61734 3.01728 9.01245C8.90553 8.7639 13.6142 6.97959 17.645 3.46819C21.6762 6.97959 26.3856 8.7641 32.2743 9.01245C32.2825 9.61715 32.2943 10.2338 32.3064 10.8701C32.5087 21.4785 32.7367 33.4877 17.645 38.9698Z"
      fill="#1727FF"
    />
    <path
      d="M22.7071 15.8924L15.6116 22.9875L12.5837 19.9597C12.0096 19.3858 11.0787 19.3858 10.5048 19.9597C9.93069 20.534 9.93069 21.4647 10.5048 22.0388L14.5721 26.1061C14.8591 26.3931 15.2354 26.5365 15.6116 26.5365C15.9877 26.5365 16.3641 26.3931 16.651 26.1061L24.7858 17.9715C25.3601 17.3974 25.3601 16.4665 24.786 15.8926C24.2121 15.3185 23.2812 15.3183 22.7071 15.8924Z"
      fill="#1727FF"
    />
  </svg>
);
...
const PricingTable = ({ region = "IN" }) => {
  ...
  const updateScrollProgress = useCallback(() => {
    ...
    if (maxScroll <= 0 || trackWidth <= 0) {
      ...
      thumbElement.style.transform = "translate3d(0px, 0px, 0px)";
      return;
    }

    const visibleRatio = clientWidth / scrollWidth;
    const thumbWidth = Math.max(trackWidth * visibleRatio, 42);
    const maxThumbX = trackWidth - thumbWidth;
    const progress = scrollLeft / maxScroll;
    const thumbX = progress * maxThumbX;

    thumbElement.style.width = `${thumbWidth}px`;
    thumbElement.style.transform = `translate3d(${thumbX}px, 0px, 0px)`;
  }, []);
  ...
  useEffect(() => {
    ...
    if (!scrollElement || !trackElement) return undefined;
  ...
  return (
    <section className="relative z-100 w-full bg-white px-[5vw] py-[7%] pb-[5vw] max-md:px-0 max-md:py-[15%]">
      <HeadingAnim>
        <h2 className="mx-auto mb-8 w-[85%] text-center text-76 font-normal leading-[1.2] text-[#071B52] max-md:mb-[8vw] max-md:text-[5vw] max-sm:mb-[10vw] max-sm:text-[9vw]">
      ...
      <div className="mb-[5vw] space-y-[1vw] max-sm:space-y-[4vw]">
        <Copy>
          <p className="mx-auto w-[70%] text-center text-24 max-md:w-[90%]">
            Most AI vendors charge more every time you add a model, deploy an
            agent, expand a workflow, or launch a new use case. What begins as a
            pilot often becomes a growing collection of licenses,
            subscriptions, and usage-based costs.
          </p>
        </Copy>
        ...
        <Copy>
          <p className="mx-auto w-[70%] text-center text-24 max-md:w-[90%]">
            With a single subscription and one governed runtime, enterprises can
            build, deploy, govern, and operate unlimited AI and Agentic AI use
            cases across the organization. No per-model pricing. No per-agent
            pricing. No penalties for scaling innovation.
          </p>
        </Copy>
      ...
      <div
        ...
        className="pricing-scroll-region fadeup w-full overflow-hidden py-[4vw] max-md:overflow-x-auto max-md:overflow-y-hidden max-md:px-[7vw] max-md:py-[10vw] max-sm:pb-[8vw]"
      >
        <div className="grid grid-cols-[2.05fr_repeat(4,1fr)] gap-[1vw] pb-[16px] max-md:min-w-[200vw] max-md:gap-[2vw] max-sm:min-w-[300vw] max-sm:grid-cols-[2.3fr_repeat(4,1.5fr)] max-sm:gap-[4vw]">
          <div className="overflow-hidden rounded-[1vw] border border-[#D9D9D9] bg-[#EAF4FF] max-md:rounded-[2vw] max-sm:rounded-[4vw]">
            <div className="flex h-[4.5vw] items-center bg-[#0205FA] px-[2vw] max-md:h-[8vw] max-sm:h-[14vw] max-sm:px-[5vw]">
              <h3 className="text-[1.35vw] font-medium text-white max-md:text-[2.5vw] max-sm:text-[5vw]">
                Inclusions
              </h3>
            </div>
            ...
            <div>
              {inclusions.map((item, index) => (
                <div
                  key={index}
                  className="flex h-[5.25vw] items-center gap-[1.6vw] border-b border-[#D9D9D9] px-[2vw] last:border-b-0 max-md:h-[9vw] max-md:gap-[3vw] max-sm:h-[17vw] max-sm:gap-[5vw] max-sm:px-[5vw]"
                >
                  <div className="shrink-0">{item.icon}</div>

                  <p className="text-[1.25vw] leading-[1.15] text-[#111111] max-md:text-[2.6vw] max-sm:text-[4.5vw]">
                    {item.label}
                  </p>
                </div>
              ))}
          ...
          {activePricing.plans.map((plan) => (
            <div
              key={plan.name}
              className="pointer-events-none relative overflow-visible rounded-[1vw] border border-[#D9D9D9] bg-white max-md:rounded-[2vw] max-sm:rounded-[4vw]"
            >
              {plan.popular && (
                <div className="absolute left-[-2.5%] top-[-5%] z-2 h-[110%] w-[105%] rounded-[1vw] border border-[#0205FA] max-sm:rounded-[4vw]">
                  <div className="absolute left-1/2 top-[-2%] z-10 flex h-[1.8vw] min-w-[9.6vw] -translate-x-1/2 items-center justify-center rounded-full border border-[#0205FA] bg-white px-[1.2vw] max-md:top-[-1.7vw] max-md:h-[3.4vw] max-md:min-w-[17vw] max-sm:top-[-3.5vw] max-sm:h-[7vw] max-sm:min-w-[35vw]">
                    <span className="text-[1vw] font-medium leading-none text-[#0205FA] max-md:text-[2vw] max-sm:text-[4vw]">
                      Most Popular
                    </span>
                  </div>
                </div>
              ...
              <div className="overflow-hidden rounded-[0.85vw] max-md:rounded-[1.8vw] max-sm:rounded-[3.6vw]">
                <div className="flex h-[4.5vw] items-center justify-center bg-[#0205FA] px-[1vw] max-md:h-[8vw] max-sm:h-[14vw]">
                  <h3 className="text-center text-[1.35vw] font-medium leading-[1.1] text-white max-md:text-[2.5vw] max-sm:text-[4.8vw]">
                    {plan.name}
                  </h3>
                </div>

                <div>
                  <div className="flex h-[5.25vw] items-center justify-center border-b border-[#D9D9D9] px-[1vw] max-md:h-[9vw] max-sm:h-[17vw]">
                    <p className="text-center text-[1.25vw] font-semibold leading-[1.15] text-[#111111] max-md:text-[2.5vw] max-sm:text-[4.6vw]">
                      {plan.useCases}
                    </p>
                  </div>

                  <div className="flex h-[5.25vw] items-center justify-center border-b border-[#D9D9D9] px-[1vw] max-md:h-[9vw] max-sm:h-[17vw]">
                    <p className="text-center text-[1.25vw] leading-[1.15] text-[#111111] max-md:text-[2.5vw] max-sm:text-[4.6vw]">
                      {getPrice(plan)}
                    </p>
                  </div>
                  ...
                  {[0, 1, 2, 3].map((item) => (
                    <div
                      key={item}
                      className="flex h-[5.25vw] items-center justify-center border-b border-[#D9D9D9] px-[1vw] last:border-b-0 max-md:h-[9vw] max-sm:h-[17vw]"
                    >
                      <CheckIcon />
                    </div>
                  ))}
      ...
      <div className="hidden px-[7vw] max-md:block">
        <div
          ref={scrollbarTrackRef}
          className="relative h-[6px] w-full overflow-hidden rounded-full bg-[#D9D9D9]"
        >
          <div
            ref={scrollbarThumbRef}
            className="pricing-scroll-thumb absolute left-0 top-0 h-full rounded-full bg-[#0205FA]"
          />
        </div>
      </div>

      <Copy>
        <div className="mt-[3vw] text-[1.35vw] leading-[1.6] text-[#111111] max-md:mt-[5vw] max-md:px-[7vw] max-md:text-[2.5vw] max-sm:mt-[6vw] max-sm:px-[5vw] max-sm:text-[4.6vw]">
