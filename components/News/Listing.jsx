...
const Listing = ({news}) => {
  return (
    <section className="px-[5vw] relative z-10" id="news-listing">
        ...
        {news.map((news,id) => (
          ...
          >
            <div className="w-full h-fit flex gap-[2.5vw] max-md:flex-col max-md:gap-[5vw]">
              ...
              <div className="w-[60%] text-foreground flex flex-col gap-[1.5vw] mt-[2vw] max-md:w-full max-md:gap-[7vw]">
                <p className="max-md:order-3 text-24 font-light">{formatDate(news.newsDate.newsDate)}</p>
                <h3 className="text-30 ordder-2 max-sm:text-[5.5vw] max-sm:w-[90%]">{news.title}</h3>
                <div className="max-md:order-1 text-24 font-light" dangerouslySetInnerHTML={{__html:news.excerpt}}/>
                <Link href={`/news/${news.slug}`} className="text-primary-blue text-24 group">
