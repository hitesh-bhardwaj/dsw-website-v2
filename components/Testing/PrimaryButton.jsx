...
const PrimaryButton = ({
  ...
  // Split text into individual letter spans (vanilla JS, no GSAP)
  useEffect(() => {
  ...
  // Native smooth scroll (no GSAP ScrollToPlugin)
  const handleClick = (e) => {
  ...
  return (
    ...
    >
      ...
      <div className="overflow-clip leading-[1.2] max-md:mx-auto -mt-0.5 max-sm:mt-0 z-1">
        <p
          ref={textRef}
          className="button-text text-22 text-white leading-[1.4] max-sm:text-[4vw]"
        >
          {text}
        </p>
      </div>

      {/* Background with hover scale */}
      <span
        className={`absolute inset-0 group-hover:scale-95 transition-transform duration-500 rounded-full bg-[#F16B0D] ${background}`}
      />
    </Link>
  );
};
