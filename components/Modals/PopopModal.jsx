...
const PopupModal = ({ modalOpen, setModalOpen }) => {
  ...
  useEffect(() => {
    ...
    if (modalOpen) {
      if (currentForm !== "book-demo") {
        params.set("form", "book-demo");
  ...
  const handleFormSuccess = useCallback(async () => {
    ...
    if (payload?.pdfUrl) {
      try {
        await downloadPdf(payload.pdfUrl, payload.fileName || "download.pdf");
      } catch (error) {
    ...
    setModalOpen(false);
  }, [payload, setFormSubmitted, setModalOpen]);

  const isPdfDownload = Boolean(payload?.pdfUrl);
  ...
  return (
    ...
    >
      <div
        data-lenis-prevent
        className="relative w-[78%] h-[78vh] max-h-[90vh] overflow-y-auto rounded-[2vw] border border-[#d4d4d4] bg-white/30 max-sm:rounded-[6vw] max-md:w-[92%] max-md:h-[78%] max-sm:h-[75%] max-sm:px-[5vw] max-sm:py-[10%] p-[4vw] max-md:pt-[6vw] max-md:p-[5vw] max-md:rounded-[4vw]"
      >
        <div className="h-full max-sm:h-fit w-full flex justify-between max-md:flex-col max-sm:gap-[7vw] max-md:gap-[4vw]">
          <div className="w-[30%] h-full space-y-[1vw] max-md:w-full max-md:space-y-[2vw]">
            {isPdfDownload ? (
              <h2 className="text-76 text-[#0A1B4B] formhead leading-[1.2]">
                Download Whitepaper
              </h2>
            ) : (
              <h2 className="text-76 text-[#0A1B4B] formhead leading-[1.2]">
          ...
          <div className="w-[55%] h-full max-md:w-full">
            {modalOpen ? (
              <DemoForm
                onSuccess={handleFormSuccess}
                submitText={isPdfDownload ? "Download Now" : "Submit"}
              />
            ) : null}
          </div>
        </div>
