...
const hasDownloadedCaseStudy = (slug) => {
...
const CaseStudySwiper = () => {
  ...
  const handleCaseStudyDownload = async (e, caseStudy) => {
    e.preventDefault();

    const caseStudySlug = slugifyCaseStudy(caseStudy.company);
    const fileName = `${caseStudy.company}-case-study.pdf`;

    if (hasDownloadedCaseStudy(caseStudySlug)) {
      try {
        await downloadPdf(caseStudy.button.href, fileName);
      } catch (err) {
    ...
    openPdf({
      pdfUrl: caseStudy.button.href,
      fileName,
      caseStudySlug,
      caseStudyName: caseStudy.company,
      formSlug: `${caseStudySlug}-casestudy`,
    });
  };
        ...
        <div className="w-full h-full gap-y-[3vw] max-md:gap-[5vw] max-sm:gap-[6vw] flex flex-col items-center text-center">
                  ...
                  <div className="flex h-full w-full max-sm:flex-col bg-white">
                    ...
                    <div className="w-[50%] max-sm:w-full p-[2.5vw] max-md:p-[3vw] flex flex-col gap-[1.5vw] justify-between text-left pb-[4vw] max-sm:p-[5vw] max-sm:gap-[4vw]">
                      ...
                      {caseStudy.button?.present && (
                        <div className="w-fit">
                          <div
                            onClick={(e) => handleCaseStudyDownload(e, caseStudy)}
          ...
          <div className="flex gap-[1vw] justify-center mt-[1vw] max-sm:mt-[6vw] max-sm:gap-[5vw] max-md:gap-[3vw]">
            ...
            <NextButton
              ...
              isDisabled={activeSlide === caseStudies.length - 1}
            />
          </div>
        </div>
      </section>
